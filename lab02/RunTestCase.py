...
class TestCaseChecker:
    ...
    def _CheckAndGetFileNames(self,programPath,lab,rollNo,ques,testcase):
        ...
        return cppFile, tcInputFile, tcOutputFile, cppOutput
    ...
    def RunTestCase(self,lab,rollNo,ques,testcase,programPath=".",compileAgain=True):
        ...
        cppFile, tcInputFile, tcOutputFile, cppOutput = self._CheckAndGetFileNames(programPath,lab,rollNo,ques,testcase)
        ...
        if not output_path.is_file() or compileAgain==True:
            compiled = self._CompileProgram(cppFile,cppOutput)
            if compiled == False:
                return 0
        ...
        #If the program does not terminate in self._timeout number of seconds
            ...
            success=0
            print(f"The program could not complete in {self._timeout} seconds.")
...
def main():
    ...
    checker = TestCaseChecker(showMessage=True,timeout=4,saveProgress=True)
    success = checker.RunTestCase(lab,rollNo,ques,testcase,programPath='.',compileAgain=True)
    if success==1:
